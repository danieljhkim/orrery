...
|---|---|---|
| Accumulated per-shell dilution Σ 1/count(k) ∝ 1/r, so "gravity = gradient of scarcity" reproduces Newton's 1/r² | supported | by construction in [scarcity-grid-weight-black-hole](../lab/sims/scarcity-grid-weight-black-hole/), [scarcity-capped-cumulative-field](../lab/sims/scarcity-capped-cumulative-field/); note: storing 1/r² and differentiating gives 1/r³ — the stored field must be potential-like (correction #1 in the thread) |
| Dividing a fixed budget across shells yields inverse-square from pure geometry (Gauss's law analog) | supported | [scarcity-shell-depletion-field](../lab/sims/scarcity-shell-depletion-field/) |
| Subtractive budget gives a hard cutoff radius r ≈ (3T/4π)^⅓ beyond which gravity dies (Yukawa-cartoon) | supported | [scarcity-capped-cumulative-field](../lab/sims/scarcity-capped-cumulative-field/), [scarcity-star-well-headroom](../lab/sims/scarcity-star-well-headroom/) — as a property of the *model*; no claim it matches nature |
| An extended (distributed) mass under the model bends rotation curves toward flat (dark-matter/MOND-adjacent) | mixed | [rotation-curve-distributed-mass](../lab/sims/rotation-curve-distributed-mass/) shows the qualitative shape; magnitudes not fit to any galaxy data — needs a study note on measured rotation curves before this can be called supported |
| Adding swirl to the scarcity field reproduces frame dragging | mixed | [frame-drag-swirl](../lab/sims/frame-drag-swirl/) — right shape; real frame dragging (Lense–Thirring) has specific magnitude/falloff this toy hasn't been checked against (`conjecture — to verify`: study note needed) |
...
Reference n-body baseline: [solar-system-nbody](../lab/sims/solar-system-nbody/) (conventional
